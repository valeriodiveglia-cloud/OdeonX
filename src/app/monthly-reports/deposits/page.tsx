...
export default function MonthlyDepositsPage() {
    ...
    const displayValue = useCallback((r: DepositRow, key: SortKey): string => {
    ...
    async function handleExport() {
        const columns: ExcelColumn[] = [
            ...
            { header: t.table.headers.remaining, key: 'remaining', width: 15, total: true, fmt: '#,##0' },
            { header: t.table.headers.status, key: 'status', width: 15 },
            { header: t.table.headers.branch, key: 'branch', width: 20 },
            { header: t.table.headers.shift, key: 'shift', width: 15 },
            { header: t.table.headers.handledBy, key: 'handledBy', width: 20 },
            { header: t.table.headers.reference, key: 'reference', width: 25 },
        ]
        ...
        await exportToExcelTable('Deposits', `deposits-${monthInputValue}.xlsx`, columns, data)
    }
        ...
        <div className="max-w-none mx-auto p-4 text-gray-100">
            ...
            <div className="mb-3 grid grid-cols-3 items-center">
                <div className="justify-self-start">
                    <button type="button" onClick={prevMonth} className="text-blue-200 hover:text-white underline underline-offset-4 decoration-blue-300/40">{t.monthNav?.previous || 'Previous'}</button>
                </div>
                ...
                <div className="justify-self-end">
                    <button type="button" onClick={nextMonth} className="text-blue-200 hover:text-white underline underline-offset-4 decoration-blue-300/40">{t.monthNav?.next || 'Next'}</button>
                </div>
            </div>
                    ...
                    <thead>
                        <tr>
                            {([
                                ...
                                ['customer', t.table.headers.customer],
                                ['amount', t.table.headers.amount, true],
                                ['paid', t.table.headers.paid || 'Paid', true],
                                ['remaining', t.table.headers.remaining, true],
                                ['status', t.table.headers.status],
                                ['branch', t.table.headers.branch],
                                ['shift', t.table.headers.shift],
                                ['handledBy', t.table.headers.handledBy],
                                ['reference', t.table.headers.reference],
                    ...
                    <tbody>
                        {filtered.length === 0 && (
                            <tr>
                                <td colSpan={10} className="text-center text-gray-500 py-6">
                                    {t.table.noRows || t.table.empty || 'No deposits found.'}
                                </td>
                            </tr>
                        )}
                        {filtered.map(r => {
                        ...
                        {filtered.length > 0 && (
                            <tr className="border-t bg-gray-50 font-semibold">
                                <td className="p-2" colSpan={2}>{t.table.totals}</td>
