...
import { computePnLData, pnlStructure, PnLLine } from './utils/pnlCalculator'
import PnLStatisticsModal from './components/PnLStatisticsModal'
...
export default function PnLReportPage() {
    ...
    }, [coa, expensesByAccount, inventoryByAccount, totalRevenue, customAdjustments, taxSettings, staffWastageByAccount, language, grossUpAccounts])
                ...
                <>
                    <div className="grid grid-cols-3 gap-4 mb-6">
                        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                            ...
                            {pnlData.revenue !== totalRevenue && (
                                <div className="text-[11px] text-slate-400 mt-2 space-y-0.5 font-medium border-t border-slate-100 pt-1.5">
                                    <div className="flex justify-between">
                                        <span>{language === 'vi' ? 'Báo cáo:' : 'Reports:'}</span>
                                        <span className="tabular-nums text-slate-600">{currency} {fmt(totalRevenue)}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span>Gross-Up:</span>
                                        <span className="tabular-nums text-slate-600">+{currency} {fmt(pnlData.revenue - totalRevenue)}</span>
                                    </div>
                                </div>
                            )}
                        </div>
                                        ...
                                        return (
                                            <tr key={i} className="hover:bg-slate-100 transition border-b border-slate-50 bg-slate-50/50">
                                                ...
                                                <td className="p-3 pl-8">
                                                    ...
                                                    {line.grossUpAmount && line.grossUpAmount > 0 && (
                                                        <div className="text-[11px] text-slate-400 mt-0.5 font-medium ml-3">
                                                            {language === 'vi' 
                                                                ? `(Báo cáo: ${fmt(line.amount - line.grossUpAmount)} | Gross-Up: +${fmt(line.grossUpAmount)})` 
                                                                : `(Reports: ${fmt(line.amount - line.grossUpAmount)} | Gross-Up: +${fmt(line.grossUpAmount)})`}
                                                        </div>
                                                    )}
                                                </td>
