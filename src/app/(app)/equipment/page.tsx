    ...
    return (
      <div className="fixed inset-0 z-[70] bg-black/60 flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 text-gray-900">
          ...
          <p className="text-sm text-gray-600 mb-4">{t('AffectNItems', lang).replace('{n}', String(count))}</p>
    ...
    return (
      <div className="fixed inset-0 z-[70] bg-black/60 flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 text-gray-900">
          ...
          <p className="text-sm text-gray-600 mb-4">{(t('AffectNItems', lang) || 'Affects {n} items').replace('{n}', String(count))}</p>

          <div className="space-y-3">
            <label className="flex items-center gap-2">
              <input type="radio" name="mk_mode" value="set" checked={mode === 'set'} onChange={() => setMode('set')} />
              <span className="text-sm">{t('SetExactValue', lang) || 'Set exact value'}</span>
            </label>

            <label className="flex items-center gap-2">
              <input type="radio" name="mk_mode" value="delta" checked={mode === 'delta'} onChange={() => setMode('delta')} />
  ...
  function EquipmentEditor(props: EditorProps) {
    const { language: lang, vatEnabled, vatRate, equipmentDefaultMarkup } = useSettings()

    // ✅ usa sempre il valore dai Settings, con fallback solo se nullo/<=0
    const defaultMarkup = useMemo(() => {
      const raw = Number(equipmentDefaultMarkup)
      return Number.isFinite(raw) && raw > 0 ? raw : 1.5
    }, [equipmentDefaultMarkup])
    ...
    const [markupStr, setMarkupStr] = useState<string>('') // NEW: markup editor
    ...
    useEffect(() => {
      if (mode === 'edit' || mode === 'view') {
        ...
        if (h.markup_x != null && isFinite(Number(h.markup_x)) && Number(h.markup_x) > 0) {
          // Priorità al valore salvato in tabella
          setMarkupStr(String(Number(h.markup_x)))
        } else {
          // Fallback: ricava dal final_price solo se markup_x manca
          ...
          if (isFinite(c) && c > 0 && isFinite(fp) && fp > 0) {
            ...
            const base = c * (1 + pct)
            const m = base > 0 ? fp / base : defaultMarkup
            setMarkupStr(String(Number.isFinite(m) ? Number(m.toFixed(3)) : defaultMarkup))
          } else {
            setMarkupStr(String(defaultMarkup))
          }
        }
      } else {
        setName('')
        ...
        setVatRatePct('')
        setViewMode(false)
        // ✅ usa il valore dai Settings anche in create
        setMarkupStr(String(defaultMarkup))
      }
    // ✅ aggiunta dipendenza defaultMarkup per aggiornarsi quando cambia nei Settings
    }, [mode, id, initial, vatEnabled, vatRate, defaultMarkup])
    ...
    async function save() {
      ...
      const pctVal = vatEnabled ? (vatRatePct.trim() === '' ? (vatRate ?? 0) : Math.max(0, Math.min(100, Number(vatRatePct)))) : 0
      ...
      const payload: any = {
        ...
        cost: cNum,
        final_price: fp, // (only for preview; DB trigger will recompute)
        markup_x: markupNum || null,
        notes: notes || null,
        last_update: new Date().toISOString(),
        vat_rate_percent: vatEnabled ? (vatRatePct.trim() === '' ? (vatRate ?? null) : Math.max(0, Math.min(100, Number(vatRatePct)))) : null,
      }

      if (id) {
        const { error } = await supabase.from(TBL_EQ).update(payload).eq('id', id)
        if (error) {
          alert(`${t('SaveFailed', lang)}: ${error.message}`)
          return
        }
        onSaved()
        return
      ...
      <Overlay onClose={onClose}>
        <div className="h-full flex flex-col text-gray-900">
          ...
          <div className="px-4 md:px-6 py-4 border-t flex items-center justify-between">
            ...
            <div>
              <button onClick={onClose} className="px-4 py-2 rounded-lg border hover:opacity-80 active:scale-95">
                {t('Close', lang)}
              </button>
              {!viewMode && (
                <button
  ...
  function ResolveImportModal(props: {
    pending: { conflicts: ConflictItem[]; rows: CsvRow[]; newValues: { categories: string[]; suppliers: string[] } }
    cats: Cat[]
    sups: Sup[]
    ...
    const catExists = (n?: string | null) => !!n && cats.some(c => c.name.toLowerCase() === String(n).toLowerCase())
    const supExists = (n?: string | null) => !!n && sups.some(s => s.name.toLowerCase() === String(n).toLowerCase())
        ...
        <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl max-h-[90vh] text-gray-900 flex flex-col">
          ...
          <div className="px-5 py-4 space-y-6 overflow-y-auto flex-1 min-h-0 max-h-[calc(90vh-120px)]">
            {!hasConflicts && !hasNewCats && !hasNewSups && (
              <div className="text-sm text-gray-600">{t('NothingToResolve', lang) || 'Nothing to resolve.'}</div>
            )}
                ...
                <div className="space-y-4">
                  {pending.conflicts.map(it => (
                    <div key={it.key} className="border rounded-2xl p-4 bg-white shadow-sm">
                      <div className="font-medium mb-3 text-gray-900">{it.name}</div>
                          ...
                          <label className="flex flex-col gap-1">
                            ...
                            <select
                              ...
                              onChange={e =>
                                setCatByKey(s => ({
                                  ...s,
                                  [it.key]: e.target.value === '' ? null : e.target.value,
                                }))
                              }
                            >
                              {(() => {
                                ...
                                return (
                                  <>
                                    ...
                                    {isCreate && (
                                      <option value={String(v)}>+ {t('AddCategory', lang)} “{createName}”</option>
                                    )}
                                    {cats.map(c => (
                              ...
                              {(() => {
                                ...
                                return (
                                  <>
                                    <option value="">{t('KeepEmpty', lang)}</option>
                                    {isCreate && (
                                      <option value={String(v)}>+ {t('AddSupplier', lang)} “{createName}”</option>
                                    )}
                                    {sups.map(s => (
                                      <option key={s.id} value={s.id}>
                                        {s.name}
            ...
            {hasNewCats && (
              <section>
                <h3 className="text-lg font-semibold mb-3">{t('NewCategoriesFromCsv', lang) || 'New categories in CSV'}</h3>
                <div className="space-y-3">
            ...
            {hasNewSups && (
              <section>
                <h3 className="text-lg font-semibold mb-3">{t('NewSuppliersFromCsv', lang) || 'New suppliers in CSV'}</h3>
                <div className="space-y-3">
  ...
  /* =====================================================
    Page
  ===================================================== */
  type SortKey = 'name' | 'category_id' | 'supplier_id' | 'cost' | 'vat_rate' | 'final_calc' | 'last_update' | 'notes'
  ...
  export default function EquipmentPage() {
    const {
      ...
      equipmentCsvConfirm,
      askCsvConfirm,
      equipmentDefaultMarkup,               // 👈 nuovo: arriva dai Settings
    } = useSettings()
  ...
  [locale]
    ...
    // ✅ defaultMarkup uniforme per tutta la pagina (listing, import, bulk…)
    const defaultMarkup = useMemo(() => {
      const raw = Number(equipmentDefaultMarkup)
      return Number.isFinite(raw) && raw > 0 ? raw : 1.5
    }, [equipmentDefaultMarkup])
    ...
    function finalCalc(e: Equip) {
      // Ricalcola SEMPRE dal costo, IVA e markup per evitare discrepanze con final_price salvato
      const c = e.cost ?? 0
    ...
    function applyFilters(list: Equip[]) {
      ...
      r.sort((a, b) => {
        const getVal = (it: Equip): any => {
          switch (sortCol) {
            case 'category_id':
              return it.category_id ?? -Infinity
            case 'supplier_id':
              return it.supplier_id ?? ''
            case 'cost':
              return it.cost ?? -Infinity
            case 'vat_rate':
              return effVatPct(it)
            case 'final_calc':
              return finalCalc(it)
            case 'last_update':
              return it.last_update ? new Date(it.last_update).getTime() : 0
            case 'notes':
              return (it.notes ?? '').toLowerCase()
            case 'name':
            default:
    ...
    async function runImport(
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        // ✅ assicurati che settings e defaultMarkup siano pronti
        if (typeof vatEnabled !== 'boolean' || !Number.isFinite(defaultMarkup) || defaultMarkup <= 0) {
        ...
        const proposed = {
          ...
          supplier_id,
          cost,
          markup_x: defaultMarkup,             // ✅ usa il valore dai Settings
        ...
        const mostRecent = candidates.length
          ? [...candidates].sort(
              (a, b) =>
                new Date(b.last_update || b.created_at).getTime() - new Date(a.last_update || a.created_at).getTime()
            )[0]
          : null
        ...
        if (existing) {
          const changed =
            ...
            (existing.markup_x ?? null) !== (proposed.markup_x ?? null) || // ✅ forza update se era NULL
          ...
          const { error } = await supabase.from(TBL_EQ).insert({ ...proposed, last_update: new Date().toISOString() })
          if (error) {
            skipped++
          } else {
            inserted++
    ...
    async function handlePickFile(e: React.ChangeEvent<HTMLInputElement>) {
      ...
      try {
        ...
        if (!data.length) {
          const samples = bad
            .slice(0, 5)
            .map(b => `r${b.row}: ${b.missing.join(', ')}`)
            .join(' | ')
          alert(
            `${
              t('CSVEmptyOrHeaders', language)
            }\n + ${t('RequiredCols', language)}: ${t('Equipment', language)}, ${t('Category', language)}, ${t(
              'Supplier',
              language
            )}, ${t('Cost', language)}. ${t('Notes', language)} ${t('IsOptional', language)}.\n + ${
        ...
        cats.forEach((c: any) => {
          catMap[toKey(c.name)] = c.id
        })
        const supMap: Record<string, string> = {}
        sups.forEach((s: any) => {
          supMap[toKey(s.name)] = s.id
        })
        ...
        for (const r of data) {
          ...
          const alreadyOk = candidates.some(
            m => (m.category_id ?? null) === (csvCatId ?? null) && (m.supplier_id ?? null) === (csvSupId ?? null)
          )
          ...
          const currentCatName =
            current.category_id != null ? (cats.find(c => c.id === current.category_id)?.name.toLowerCase() || null) : null
          const currentSupName =
            current.supplier_id != null ? (sups.find(s => s.id === current.supplier_id)?.name.toLowerCase() || null) : null
        ...
        if (!needConfirm) {
          try {
            if (newCats.length > 0) {
              const { error: insCatsErr } = await supabase.from(TBL_EQ_CATS).insert(newCats.map(n => ({ name: capitalizeFirst(n) })))
              if (insCatsErr) throw insCatsErr
            }
            if (newSups.length > 0) {
              const { error: insSupsErr } = await supabase.from(TBL_SUPS).insert(newSups.map(n => ({ name: capitalizeFirst(n) })))
            ...
            ;(cRes2.data || cats).forEach((c: any) => {
              catMap2[c.name.toLowerCase()] = c.id
            })
            ;(sRes2.data || sups).forEach((s: any) => {
              supMap2[s.name.toLowerCase()] = s.id
            })
    ...
    async function onUnifiedConfirm(choice: UnifiedChoice) {
      ...
      try {
        ...
        sups.forEach((s: any) => {
          supMap[s.name.toLowerCase()] = s.id
        })
        insCats?.forEach((c: any) => {
          catMap[c.name.toLowerCase()] = c.id
        })
        insSups?.forEach((s: any) => {
          supMap[s.name.toLowerCase()] = s.id
        })
    ...
    async function handleExportExcel() {
      try {
        ...
        const vatCols = vatEnabled ? [{ header: t('VatRatePct', language), key: 'VatRatePct', width: 12 }] : []
        const tailCols = [
    ...
    async function bulkMarkReviewed() {
      ...
      if (error) {
        alert(`${t('FailedToMarkReviewed', language)}: ${error.message}`)
        return
      }
      await fetchAll()
    }
    ...
    async function bulkMoveToTrash() {
      ...
      if (error) {
        alert(`${t('FailedToMoveTrash', language)}: ${error.message}`)
        return
      }
      await fetchAll()
    }
      ...
      try {
        ...
        await Promise.all(
          selectedRows.map(async m => {
            ...
            const { error } = await supabase.from(TBL_EQ).update({ vat_rate_percent: newVal, last_update: now }).eq('id', m.id)
      ...
      try {
        ...
        await Promise.all(
          selectedRows.map(async m => {
            ...
            const payload: any = {
              markup_x: newMarkup,
              final_price: isFinite(newFinal) ? newFinal : null, // DB trigger rifarà i conti
              last_update: now,
            }
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={exporting}
              className="inline-flex items-center gap-2 px-3 h-9 rounded-lg bg-blue-600/15 text-blue-200 hover:bg-blue-600/25 border border-blue-400/30 disabled:opacity-60"
            ...
            <button onClick={openCreate} className="inline-flex items-center gap-2 px-3 h-9 rounded-lg bg-blue-600 text-white hover:opacity-80">
              <PlusIcon className="w-5 h-5" />
              {t('NewEquipment', language)}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow p-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 px-3 h-9 rounded-lg border border-blue-600 text-blue-700 hover:bg-blue-50"
  ...
  {filtered.map(it => {
    ...
    return (
      <tr
        key={it.id}
        className={`border-t hover:bg-blue-50/40 ${isSelected ? 'bg-blue-100/70' : ''} cursor-pointer`}
        onClick={() => openView(it)}
      ...
      >
        <td className="p-2" onClick={e => e.stopPropagation()}>
          {selectMode && (
            <input
              ...
              title={t('SelectRow', language)}
            />
          )}
        </td>

        {/* Nome: niente onClick qui, la riga è già cliccabile */}
        <td className="p-2">{it.name}</td>

        <td className="p-2">{cats.find(c => c.id === it.category_id)?.name || ''}</td>
        <td className="p-2">{sups.find(s => s.id === it.supplier_id)?.name || ''}</td>

        <td className="p-2 text-right">{it.cost != null ? num.format(it.cost) : ''}</td>

        {vatEnabled && <td className="p-2 text-right">{effVatPct(it)}%</td>}

        <td className="p-2 text-right font-semibold">{num.format(finalCalc(it))}</td>

        <td
          className={`p-2 text-right ${isOlderThanMonths(it.last_update, reviewM) ? 'text-red-600 font-medium' : ''}`}
        >
          {fmtDate(it.last_update)}
        </td>

        <td className="p-2 truncate max-w-[200px]">{it.notes || ''}</td>
      </tr>
    )
  })}
</tbody>
        ...
        {openEditor && (
  <EquipmentEditor
    key={`ed-${equipmentDefaultMarkup}-${editorMode}-${editingId ?? 'new'}`} // 👈 forza remount quando cambia il default
    mode={editorMode}
    id={editingId}
    cats={cats}
    sups={sups}
