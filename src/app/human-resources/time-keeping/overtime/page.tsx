...
import { Users, Watch, X, Loader2, Save, Trash2, CalendarDays, Plus, ChevronDown, ArrowUp, ArrowDown, Filter, MoreVertical, Search } from 'lucide-react'
...
export default function OvertimeMonthlyPage() {
    ...
    const uniquePositions = useMemo(() => {
        return Array.from(new Set(staffWithOvertime.map(s => s.position).filter(Boolean))) as string[]
    }, [staffWithOvertime])

    const sortedAndFilteredStaff = useMemo(() => {
        let list = [...staffWithOvertime]
        ...
        return list
    }, [staffWithOvertime, searchQuery, filterPosition, sortKey, sortAsc])
                                ...
                                {sortedAndFilteredStaff.length === 0 && (
                                    <tr>
                                        <td colSpan={6} className="text-center py-8 text-slate-400 text-xs italic font-semibold">
                                            {language === 'vi' ? 'Không có dữ liệu tăng ca' : 'No overtime data found'}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
