...
export default function CRMCommissionsPage() {
    ...
    const fetchPayouts = async () => {
        ...
        if (error) {
            ...
            setPayouts(data as unknown as ExtendedPayout[])
        }
        setLoading(false)
    }
    ...
    /* Generate automatic payouts from Validated referrals */
    const generatePayouts = async () => {
        if (!confirm(`Are you sure you want to generate payouts for all pending referrals in ${formatMonthLabel(monthCursor)}?`)) return
        ...
        try {
            ...
            // Group by partner_id
            const partnerGroups: Record<string, { amount: number, refIds: string[] }> = {}
            for (const r of refs || []) {
                if (!partnerGroups[r.partner_id]) {
                    partnerGroups[r.partner_id] = { amount: 0, refIds: [] }
                }
            ...
            if (Object.keys(partnerGroups).length === 0) {
                alert(`No pending unassigned referrals found to generate payouts for ${formatMonthLabel(monthCursor)}.`)
                setIsGenerating(false)
                return
            }

            // Insert new payouts and bind payout_id
            let insertedCount = 0
            for (const [partnerId, group] of Object.entries(partnerGroups)) {
                ...
                // Bind referrals to this new payout
            ...
            if (insertedCount > 0) {
                ...
                alert('No payouts were generated.')
            }
    ...
    const submitMarkPaid = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Also update ONLY the underlying referrals bound to this exact payout
    ...
    const filteredPayouts = useMemo(() => {
        return payouts.filter(p => {
            ...
            const monthLabel = formatMonthLabel(monthCursor).toLowerCase()
    ...
    }, [payouts, searchTerm, monthCursor])
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto relative">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-6 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900">Commissions & Payouts</h1>
                    <p className="text-slate-500 mt-1">Manage partner earnings and record commission payments.</p>
                </div>
                ...
                <table className="w-full table-auto text-sm text-gray-900 text-left border-collapse">
                    <thead>
                        <tr className="text-gray-500 font-semibold">
                            <th className="p-2 whitespace-nowrap">Payout ID</th>
                            <th className="p-2 whitespace-nowrap">Partner</th>
                            <th className="p-2 whitespace-nowrap">Period</th>
                            <th className="p-2 whitespace-nowrap text-right">Amount ({currency})</th>
                            ...
                            filteredPayouts.map(payout => (
                                <tr key={payout.id} className="border-t hover:bg-blue-50/40 transition">
                                    <td className="p-2 whitespace-nowrap">
                                        <div className="font-semibold text-slate-900 font-mono text-xs uppercase">{payout.id.split('-')[0]}</div>
                                    </td>
                                    <td className="p-2 whitespace-nowrap">
                                        <div className="font-semibold text-gray-900">{payout.crm_partners?.name || 'Unknown Partner'}</div>
                                    </td>
                                    <td className="p-2 whitespace-nowrap">
                                        <div className="text-gray-700">{payout.period}</div>
                                    </td>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100">
                            <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                                ...
                                {modalMode === 'markPaid' ? 'Mark Payout as Paid' : 'Payout Receipt'}
                            </h2>
                            ...
                            <form onSubmit={submitMarkPaid} className="p-4 sm:p-6 flex flex-col gap-5">
                                ...
                                <div className="bg-blue-50 p-4 rounded-xl flex justify-between items-center border border-blue-100">
                                    <div className="text-sm font-medium text-blue-800">
                                        Partner: <span className="font-bold">{selectedPayout.crm_partners?.name || 'Unknown'}</span>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <textarea 
                                        rows={3}
                                        placeholder="Add any internal details about this transaction..."
                                        value={paymentNotes}
                        ...
                        {modalMode === 'viewReceipt' && (
                            <div className="p-4 sm:p-6 flex flex-col gap-6">
                                <div className="text-center">
                                    ...
                                    <p className="text-sm font-semibold text-emerald-600 mt-1 uppercase tracking-wider">Transaction Successful</p>
                                </div>
                                ...
                                <div className="bg-slate-50 rounded-xl border border-slate-100 p-4 space-y-4">
                                    <div className="flex justify-between items-center border-b border-slate-200/60 pb-3">
                                        ...
                                        <span className="text-sm font-semibold text-slate-900">{selectedPayout.crm_partners?.name || 'Unknown'}</span>
                                    </div>
                                    <div className="flex justify-between items-center border-b border-slate-200/60 pb-3">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-sm font-mono font-semibold text-slate-900">{selectedPayout.reference_number || 'Cash'}</span>
                                    </div>
                                </div>
