...
import { Clock, Timer, Coins, Home } from 'lucide-react'
import { useSettings } from '@/contexts/SettingsContext'
import ReactCountryFlag from 'react-country-flag'

const BASE = '/human-resources/time-keeping'
...
export default function LeftNavTimeKeeping() {
    ...
    const headerTitle = language === 'vi' ? 'Chấm công' : 'Time Keeping'

    const NAV = [
        { href: BASE, label: language === 'vi' ? 'Bảng điều khiển' : 'Dashboard', icon: Home, exact: true },
        { href: `${BASE}/attendance`, label: language === 'vi' ? 'Điểm danh' : 'Attendance', icon: Clock },
        { href: `${BASE}/overtime`, label: language === 'vi' ? 'Tăng ca' : 'Overtime', icon: Timer },
        { href: `${BASE}/service-charge`, label: language === 'vi' ? 'Phí dịch vụ' : 'Service Charge', icon: Coins },
    ]
