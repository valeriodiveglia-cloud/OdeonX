                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="border-t border-gray-100 pt-6 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500">Current Base</h4>
                                <div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Previous Type</label>
                                    <input type="text" readOnly value={prevSalaryType === 'hourly' ? 'Hourly' : 'Fixed / Month'}
                                        className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-500 bg-gray-50 outline-none cursor-not-allowed" />
                                </div>
                            </div>
                            ...
                            {/* New Salary */}
                            <div className="space-y-4">
                                <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500">New Gross Salary Details</h4>
                                <div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">New Type *</label>
                        ...
                        {!typeChanged && (
                            <div className="pt-2 grid grid-cols-1 sm:grid-cols-2 gap-6 items-start">
                                {/* Left side: Toggle */}
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1 opacity-0 hidden sm:block">Calculate</label>
                                        ...
                                        <>
                                            ...
                                            <div className="relative w-full">
                                                <input type="number" step="any" value={increaseValue} onChange={e => setIncreaseValue(e.target.value)}
                                                    ...
                                                    placeholder={increaseType === 'percentage' ? "e.g. 5" : "e.g. 500000"} />
                    ...
                    {changePreview && Number(changePreview) !== 0 && (
                        <div className={`flex items-center gap-2 p-3 rounded-lg ${Number(changePreview) > 0 ? 'bg-emerald-50' : 'bg-red-50'}`}>
                            ...
                            <span className={`text-sm font-medium ${Number(changePreview) > 0 ? 'text-emerald-700' : 'text-red-700'}`}>
                                ...
                                ({fmt(Number(newAmount.replace(/,/g, '')) - Number(prevAmount.replace(/,/g, '')))} VND)
                            </span>
                        </div>
                    )}
                        </>
                    ...
                    <div className="flex items-center justify-between gap-3 pt-2 border-t border-gray-100">
                        <div className="text-xs font-medium text-gray-400">
                            {approvedBy && <span>Approved by: <span className="text-gray-500">{approvedBy}</span></span>}
                        </div>
