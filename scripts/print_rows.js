...
async function run() {
    const { data: closings } = await supabase.from('cashier_closings').select('report_date, branch_name, opening_float_vnd, cash_json, float_plan_json').lt('report_date', '2026-05-13')
    ...
    for (const c of branchClosings) {
        ...
        if (cashToTake > 0) {
            ...
            if (deposited < cashToTake) {
                console.log(`Date: ${cDate}, Cash To Take: ${cashToTake}, Deposited: ${deposited}`)
            }
        }
    }
}
