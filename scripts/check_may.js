...
async function run() {
    const { data: allClosings } = await supabase.from('cashier_closings').select('report_date, branch_name, opening_float_vnd, cash_json, float_plan_json').eq('branch_name', 'Pasta Fresca Thao Dien').gte('report_date', '2026-05-01')
    ...
    console.log("May cash to take for Thao Dien:", sum)
}
run()
