-- Migration: Notify on Cashier Closing Update (Option 2)
-- Description:
-- 1. Updates fn_trigger_daily_reports_notifications() to check for an existing daily_reports notification in the last 24 hours for the same branch and update it rather than inserting a duplicate.
-- 2. Alters the trigger tr_daily_reports_closing_notifications to trigger AFTER INSERT OR UPDATE.
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'cashier_closings' AND TG_OP IN ('INSERT', 'UPDATE') THEN
        ...
        WHERE module = 'daily_reports' 
          AND branch_id = NEW.branch_id::text
    ...
    -- Nuovo deposito registrato (rimane invariato)
...
-- Ricrea il trigger per attivarsi anche su UPDATE
...
AFTER INSERT OR UPDATE ON public.cashier_closings
FOR EACH ROW
EXECUTE FUNCTION fn_trigger_daily_reports_notifications();
