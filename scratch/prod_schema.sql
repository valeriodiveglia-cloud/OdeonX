...
CREATE OR REPLACE FUNCTION "public"."event_discount_rows_ensure_header"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  insert into public.event_headers (id)
  values (NEW.event_id)
...
ALTER FUNCTION "public"."event_discount_rows_ensure_header"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_price_after_discounts" bigint) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_price_after_discounts" bigint) OWNER TO "postgres";
...
begin
  -- Server-side aggregation (trust no derived client fields)
...
CREATE OR REPLACE FUNCTION "public"."fn_check_month_lock"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $_$
DECLARE
  v_date DATE;
  v_month TEXT;
  v_col TEXT := TG_ARGV[0];  -- e.g. 'date' or 'report_date'
BEGIN
  -- For DELETE, check old row; for INSERT/UPDATE, check new row
  IF TG_OP = 'DELETE' THEN
    EXECUTE format('SELECT ($1).%I', v_col) INTO v_date USING OLD;
  ELSE
  ...
  IF EXISTS (SELECT 1 FROM month_locks WHERE month_key = v_month) THEN
    RAISE EXCEPTION 'Month % is locked. Modifications are not allowed.', v_month
      USING ERRCODE = 'P0001';
  END IF;
...
ALTER FUNCTION "public"."fn_check_month_lock"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_audit_log"("p_start" timestamp with time zone, "p_end" timestamp with time zone, "p_table" "text" DEFAULT NULL::"text", "p_op" "text" DEFAULT NULL::"text", "p_limit" integer DEFAULT 2000) RETURNS SETOF "public"."audit_log"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  -- Strategy 1: direct user_id match in app_accounts
  SELECT lower(a.role) INTO v_role
  FROM app_accounts a
  WHERE a.user_id = v_uid AND a.is_active = true
  LIMIT 1;

  -- Strategy 2: if not found, look up email from auth.users, then match by email
  IF v_role IS NULL THEN
    ...
    IF v_email IS NOT NULL THEN
      SELECT lower(a.role) INTO v_role
      FROM app_accounts a
      WHERE lower(a.email) = lower(v_email) AND a.is_active = true
      LIMIT 1;
    END IF;
  END IF;
...
ALTER FUNCTION "public"."get_audit_log"("p_start" timestamp with time zone, "p_end" timestamp with time zone, "p_table" "text", "p_op" "text", "p_limit" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."rental_equipment_apply_pricing"() RETURNS "trigger"
    ...
    AS $$
DECLARE
  v_cost numeric := COALESCE(NEW.cost, 0);
  v_vat  numeric := COALESCE(NEW.vat_rate_percent, 0);
  v_mkp  numeric := COALESCE(NEW.markup_x, 1.5);
BEGIN
  NEW.final_price := ROUND( (v_cost * (1 + v_vat/100.0)) * v_mkp, 0 );
  NEW.last_update := NOW();
  RETURN NEW;
END;
$$;
...
CREATE OR REPLACE FUNCTION "public"."set_updated_at_tg"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
begin
  new.updated_at := now();
...
ALTER FUNCTION "public"."set_updated_at_tg"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."toggle_month_lock"("p_month_key" "text") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_email TEXT;
...
BEGIN
  ...
  -- Check role via app_accounts
  SELECT role INTO v_role FROM app_accounts WHERE email = v_email LIMIT 1;
  IF v_role IS NULL OR v_role NOT IN ('owner', 'manager') THEN
    RAISE EXCEPTION 'Only owner or manager can lock/unlock months';
  END IF;

  -- Check current state
  SELECT EXISTS (SELECT 1 FROM month_locks WHERE month_key = p_month_key) INTO v_is_locked;

  IF v_is_locked THEN
    DELETE FROM month_locks WHERE month_key = p_month_key;
    RETURN jsonb_build_object('locked', false, 'month_key', p_month_key);
  ELSE
    INSERT INTO month_locks (month_key, locked_by) VALUES (p_month_key, auth.uid());
    RETURN jsonb_build_object('locked', true, 'month_key', p_month_key);
  END IF;
END;
$$;


ALTER FUNCTION "public"."toggle_month_lock"("p_month_key" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."transport_propagate_markup"("p_event_id" "text", "p_markup" numeric) RETURNS "void"
    LANGUAGE "sql"
    AS $$
  update public.event_transport_rows r
  set markup_x = p_markup
  where r.event_id = p_event_id;
$$;


ALTER FUNCTION "public"."transport_propagate_markup"("p_event_id" "text", "p_markup" numeric) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."transport_replace_vehicle_types"("p_event_id" "text", "p_names" "text"[], "p_costs" numeric[], "p_positions" smallint[] DEFAULT NULL::smallint[]) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
declare
  ...
  pos int2;
begin
  if n <> coalesce(array_length(p_costs, 1), 0) then
    raise exception 'Length mismatch: names vs costs';
  end if;

  delete from public.transport_vehicle_types where event_id = p_event_id;

  i := 1;
  while i <= n loop
    pos := coalesce(p_positions[i], i-1);
    insert into public.transport_vehicle_types(event_id, name, cost_per_km, position)
    values (p_event_id, p_names[i], coalesce(p_costs[i], 0), pos);
    i := i + 1;
  end loop;
end;
$$;
...
ALTER FUNCTION "public"."transport_replace_vehicle_types"("p_event_id" "text", "p_names" "text"[], "p_costs" numeric[], "p_positions" smallint[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."transport_rows_apply_default_markup"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
declare
  m numeric;
begin
  if new.markup_x is null then
    select ts.markup_x into m
...
ALTER FUNCTION "public"."transport_rows_apply_default_markup"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."transport_settings_upsert"("p_event_id" "text", "p_markup_x" numeric) RETURNS "void"
    LANGUAGE "sql"
    AS $$
  insert into public.transport_settings(event_id, markup_x)
  values (p_event_id, coalesce(p_markup_x, 1.0))
  on conflict (event_id)
  do update set markup_x = excluded.markup_x, updated_at = now();
$$;


ALTER FUNCTION "public"."transport_settings_upsert"("p_event_id" "text", "p_markup_x" numeric) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."update_hr_staff_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  NEW.updated_at = NOW();
...
ALTER FUNCTION "public"."update_hr_staff_updated_at"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."cashout" (
    ...
    "branch" "text",
    "supplier_name" "text",
    CONSTRAINT "cashout_amount_check" CHECK (("amount" >= 0))
);
...
CREATE TABLE IF NOT EXISTS "public"."event_headers" (
    ...
    "balance_percent" numeric DEFAULT 100,
...
COMMENT ON TABLE "public"."contract_templates" IS 'Stores DOCX templates and configuration for event contracts';
...
CREATE TABLE IF NOT EXISTS "public"."equipment_price_history" (
    ...
    CONSTRAINT "equipment_price_history_change_type_ck" CHECK (("change_type" = ANY (ARRAY['insert'::"text", 'update'::"text", 'delete'::"text", 'snapshot'::"text"])))
);
...
CREATE OR REPLACE VIEW "public"."final_recipe_tags_vw" WITH ("security_invoker"='on') AS
 SELECT "frt"."final_id",
    ...
    "string_agg"("t"."name", ', '::"text" ORDER BY "t"."name") AS "tags_text"
   FROM ("public"."final_recipe_tags" "frt"
     JOIN "public"."tags" "t" ON (("t"."id" = "frt"."tag_id")))
  GROUP BY "frt"."final_id";
...
CREATE TABLE IF NOT EXISTS "public"."hr_staff" (
    ...
    "salary_amount" numeric(15,2) DEFAULT 0,
    "start_date" "date",
    "status" "text" DEFAULT 'active'::"text" NOT NULL,
    "notes" "text",
    "created_at" timestamp with time zone DEFAULT "now"(),
...
CREATE TABLE IF NOT EXISTS "public"."materials" (
    ...
    CONSTRAINT "materials_package_price_req_chk" CHECK (("package_price" >= (0)::numeric)),
    CONSTRAINT "materials_packaging_size_req_chk" CHECK (("packaging_size" > (0)::numeric)),
...
ALTER TABLE "public"."month_locks" OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "public"."prep_list_vw" WITH ("security_invoker"='on') AS
 SELECT "p"."id",
    ...
    COALESCE("p"."last_update", (("now"() AT TIME ZONE 'utc'::"text"))::timestamp with time zone) AS "last_update"
   FROM ("public"."prep_recipes" "p"
     LEFT JOIN "public"."recipe_categories" "rc" ON (("rc"."id" = "p"."category_id")));
...
ALTER TABLE ONLY "public"."app_accounts"
    ADD CONSTRAINT "app_accounts_email_lower_uk" UNIQUE ("email_lower");
...
ALTER TABLE ONLY "public"."event_staff_settings"
    ADD CONSTRAINT "event_staff_settings_event_id_uidx" UNIQUE ("event_id");
...
ALTER TABLE ONLY "public"."materials"
    ADD CONSTRAINT "materials_name_supplier_brand_key" UNIQUE ("name", "supplier_id", "brand_key");
...
ALTER TABLE ONLY "public"."month_locks"
    ADD CONSTRAINT "month_locks_pkey" PRIMARY KEY ("month_key");
...
ALTER TABLE ONLY "public"."rental_equipment"
    ADD CONSTRAINT "rental_equipment_id_key" UNIQUE ("id");
...
CREATE INDEX "idx_audit_log_rowpk_time" ON "audit"."log" USING "btree" ("table_name", "row_pk", "changed_at" DESC);



CREATE INDEX "idx_audit_log_table_time" ON "audit"."log" USING "btree" ("table_name", "changed_at" DESC);



CREATE UNIQUE INDEX "app_accounts_user_id_uidx" ON "public"."app_accounts" USING "btree" ("user_id") WHERE ("user_id" IS NOT NULL);
...
CREATE INDEX "cashout_category_idx" ON "public"."cashout" USING "btree" ("category");



CREATE INDEX "cashout_date_idx" ON "public"."cashout" USING "btree" ("date");



CREATE INDEX "cashout_delivery_note_idx" ON "public"."cashout" USING "btree" ("delivery_note");



CREATE INDEX "cashout_invoice_idx" ON "public"."cashout" USING "btree" ("invoice");



CREATE INDEX "cashout_supplier_id_idx" ON "public"."cashout" USING "btree" ("supplier_id");



CREATE UNIQUE INDEX "categories_name_ci_uidx" ON "public"."categories" USING "btree" ("lower"("name"));
...
CREATE UNIQUE INDEX "contract_templates_key_uk" ON "public"."contract_templates" USING "btree" ("lower"("key"));



CREATE UNIQUE INDEX "contract_templates_key_uniq" ON "public"."contract_templates" USING "btree" ("lower"("key"));
...
CREATE UNIQUE INDEX "dish_categories_name_ci_uidx" ON "public"."dish_categories" USING "btree" ("lower"("name"));



CREATE UNIQUE INDEX "equipment_categories_name_ci_uidx" ON "public"."equipment_categories" USING "btree" ("lower"("name"));



CREATE INDEX "equipment_price_history_changed_at_idx" ON "public"."equipment_price_history" USING "btree" ("changed_at" DESC);



CREATE INDEX "equipment_price_history_equipment_id_idx" ON "public"."equipment_price_history" USING "btree" ("equipment_id");



CREATE UNIQUE INDEX "final_recipes_name_ci_uidx" ON "public"."final_recipes" USING "btree" ("lower"("name"));
...
CREATE UNIQUE INDEX "prep_recipes_name_ci_uidx" ON "public"."prep_recipes" USING "btree" ("lower"("name"));



CREATE UNIQUE INDEX "recipe_categories_name_ci_uidx" ON "public"."recipe_categories" USING "btree" ("lower"("name"));
...
CREATE UNIQUE INDEX "tags_name_unique_idx" ON "public"."tags" USING "btree" ("lower"("name"));
...
CREATE OR REPLACE TRIGGER "event_discount_rows_ensure_header" BEFORE INSERT ON "public"."event_discount_rows" FOR EACH ROW EXECUTE FUNCTION "public"."event_discount_rows_ensure_header"();



CREATE OR REPLACE TRIGGER "hr_staff_updated_at" BEFORE UPDATE ON "public"."hr_staff" FOR EACH ROW EXECUTE FUNCTION "public"."update_hr_staff_updated_at"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_bundle_rows" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_bundles" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_company_asset_rows" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_equipment_rows" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_extra_fee_rows" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_transport_rows" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_transport_settings" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."event_transport_vehicle_types" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."provider_branches" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "set_timestamp" BEFORE UPDATE ON "public"."transport_defaults" FOR EACH ROW EXECUTE FUNCTION "public"."trigger_set_timestamp"();



CREATE OR REPLACE TRIGGER "trg_audit_cashier_closings" AFTER INSERT OR DELETE OR UPDATE ON "public"."cashier_closings" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_audit_cashout" AFTER INSERT OR DELETE OR UPDATE ON "public"."cashout" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();
...
CREATE OR REPLACE TRIGGER "trg_audit_credit_payments" AFTER INSERT OR DELETE OR UPDATE ON "public"."credit_payments" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_audit_credits" AFTER INSERT OR DELETE OR UPDATE ON "public"."credits" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_audit_daily_report_bank_transfers" AFTER INSERT OR DELETE OR UPDATE ON "public"."daily_report_bank_transfers" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_audit_daily_report_settings" AFTER INSERT OR DELETE OR UPDATE ON "public"."daily_report_settings" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_audit_deposit_payments" AFTER INSERT OR DELETE OR UPDATE ON "public"."deposit_payments" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_audit_deposits" AFTER INSERT OR DELETE OR UPDATE ON "public"."deposits" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();
...
CREATE OR REPLACE TRIGGER "trg_audit_uom" AFTER INSERT OR DELETE OR UPDATE ON "public"."uom" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_audit_wastage_entries" AFTER INSERT OR DELETE OR UPDATE ON "public"."wastage_entries" FOR EACH ROW EXECUTE FUNCTION "public"."fn_audit_row"();



CREATE OR REPLACE TRIGGER "trg_cashout_updated_at" BEFORE UPDATE ON "public"."cashout" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();



CREATE OR REPLACE TRIGGER "trg_contract_template_settings_updated_at" BEFORE UPDATE ON "public"."contract_template_settings" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_event_payment_state_updated_at" BEFORE UPDATE ON "public"."event_payment_state" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_log_material_price_change" AFTER UPDATE OF "package_price", "packaging_size", "unit_cost" ON "public"."materials" FOR EACH ROW WHEN ((("old"."package_price" IS DISTINCT FROM "new"."package_price") OR ("old"."packaging_size" IS DISTINCT FROM "new"."packaging_size") OR ("old"."unit_cost" IS DISTINCT FROM "new"."unit_cost"))) EXECUTE FUNCTION "public"."log_material_price_change"();



CREATE OR REPLACE TRIGGER "trg_month_lock" BEFORE INSERT OR DELETE OR UPDATE ON "public"."cashier_closings" FOR EACH ROW EXECUTE FUNCTION "public"."fn_check_month_lock"('report_date');



CREATE OR REPLACE TRIGGER "trg_month_lock" BEFORE INSERT OR DELETE OR UPDATE ON "public"."cashout" FOR EACH ROW EXECUTE FUNCTION "public"."fn_check_month_lock"('date');



CREATE OR REPLACE TRIGGER "trg_month_lock" BEFORE INSERT OR DELETE OR UPDATE ON "public"."daily_report_bank_transfers" FOR EACH ROW EXECUTE FUNCTION "public"."fn_check_month_lock"('date');



CREATE OR REPLACE TRIGGER "trg_month_lock" BEFORE INSERT OR DELETE OR UPDATE ON "public"."wastage_entries" FOR EACH ROW EXECUTE FUNCTION "public"."fn_check_month_lock"('date');
...
ALTER TABLE ONLY "public"."event_discount_rows"
    ADD CONSTRAINT "event_discount_rows_event_id_fkey" FOREIGN KEY ("event_id") REFERENCES "public"."event_headers"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."event_payment_state"
    ADD CONSTRAINT "fk_event_payment_state_event" FOREIGN KEY ("event_id") REFERENCES "public"."event_headers"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."month_locks"
    ADD CONSTRAINT "month_locks_locked_by_fkey" FOREIGN KEY ("locked_by") REFERENCES "auth"."users"("id");
...
CREATE POLICY "Admin/Owner can delete" ON "public"."month_locks" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'owner'::"text"]))))));



CREATE POLICY "Admin/Owner can modify" ON "public"."month_locks" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'owner'::"text"]))))));



CREATE POLICY "Admin/Owner can update" ON "public"."month_locks" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'owner'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'owner'::"text"]))))));
...
CREATE POLICY "All app users can read" ON "public"."month_locks" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
...
CREATE POLICY "Allow insert/update for service role" ON "public"."event_payment_state" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "Allow read access for authenticated users" ON "public"."event_payment_state" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "App users full access" ON "public"."contract_template_settings" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"]))))));
...
CREATE POLICY "App users full access" ON "public"."crm_tasks" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text", 'sale advisor'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text", 'sale advisor'::"text"]))))));
...
CREATE POLICY "App users full access" ON "public"."event_discount_rows" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"]))))));
...
CREATE POLICY "App users full access" ON "public"."hr_staff" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"]))))));
...
CREATE POLICY "App users full access" ON "public"."hr_staff_branches" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"]))))));
...
CREATE POLICY "App users full access" ON "public"."provider_branches" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."app_accounts"
  WHERE (("app_accounts"."user_id" = "auth"."uid"()) AND ("app_accounts"."role" = ANY (ARRAY['admin'::"text", 'staff'::"text", 'manager'::"text", 'owner'::"text"]))))));
...
CREATE POLICY "app_accounts_del_admin_owner" ON "public"."app_accounts" FOR DELETE TO "authenticated" USING (("public"."app_is_owner"() OR ("public"."app_is_admin"() AND ("role" = ANY (ARRAY['admin'::"text", 'staff'::"text"])))));



CREATE POLICY "app_accounts_ins_admin" ON "public"."app_accounts" FOR INSERT TO "authenticated" WITH CHECK (("public"."app_is_admin"() AND ("role" = 'staff'::"text")));



CREATE POLICY "app_accounts_ins_admin_owner" ON "public"."app_accounts" FOR INSERT TO "authenticated" WITH CHECK (("public"."app_is_owner"() OR ("public"."app_is_admin"() AND ("role" = ANY (ARRAY['admin'::"text", 'staff'::"text"])))));



CREATE POLICY "app_accounts_ins_authadmin" ON "public"."app_accounts" FOR INSERT TO "supabase_auth_admin" WITH CHECK (true);



CREATE POLICY "app_accounts_ins_owner" ON "public"."app_accounts" FOR INSERT TO "authenticated" WITH CHECK ("public"."app_is_owner"());



CREATE POLICY "app_accounts_ins_servicerole" ON "public"."app_accounts" FOR INSERT TO "service_role" WITH CHECK (true);
...
CREATE POLICY "app_accounts_upd_admin_owner" ON "public"."app_accounts" FOR UPDATE TO "authenticated" USING (("public"."app_is_owner"() OR ("public"."app_is_admin"() AND ("role" = ANY (ARRAY['admin'::"text", 'staff'::"text"]))))) WITH CHECK (("public"."app_is_owner"() OR ("public"."app_is_admin"() AND ("role" = ANY (ARRAY['admin'::"text", 'staff'::"text"])))));
...
CREATE POLICY "app_settings_ins_authenticated" ON "public"."app_settings" FOR INSERT TO "authenticated" WITH CHECK (true);



CREATE POLICY "app_settings_insert" ON "public"."app_settings" FOR INSERT TO "authenticated" WITH CHECK ((("id" = 'singleton'::"text") AND (EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("a"."role" = ANY (ARRAY['owner'::"text", 'admin'::"text"])))))));
...
CREATE POLICY "app_settings_upd_authenticated" ON "public"."app_settings" FOR UPDATE TO "authenticated" USING (true) WITH CHECK (true);



CREATE POLICY "app_settings_update" ON "public"."app_settings" FOR UPDATE TO "authenticated" USING ((("id" = 'singleton'::"text") AND (EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("a"."role" = ANY (ARRAY['owner'::"text", 'admin'::"text"]))))))) WITH CHECK ((("id" = 'singleton'::"text") AND (EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("a"."role" = ANY (ARRAY['owner'::"text", 'admin'::"text"])))))));
...
CREATE POLICY "categories_sel_anon" ON "public"."categories" FOR SELECT TO "anon" USING (true);
...
CREATE POLICY "contract_templates_mod" ON "public"."contract_templates" TO "authenticated" USING (true) WITH CHECK (true);



CREATE POLICY "contract_templates_sel" ON "public"."contract_templates" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "contract_templates_sel_active" ON "public"."contract_templates" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("lower"("a"."role") = ANY (ARRAY['staff'::"text", 'admin'::"text", 'owner'::"text"]))))));



CREATE POLICY "contract_templates_select_public" ON "public"."contract_templates" FOR SELECT TO "anon" USING (true);
...
CREATE POLICY "del_authenticated" ON "public"."suppliers" FOR DELETE TO "authenticated" USING (true);
...
CREATE POLICY "eh select authenticated" ON "public"."event_headers" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "eps select authenticated" ON "public"."event_payment_state" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "et select authenticated" ON "public"."event_totals" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "event_bundle_rows_mod_active" ON "public"."event_bundle_rows" TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("lower"("a"."role") = ANY (ARRAY['staff'::"text", 'admin'::"text", 'owner'::"text"]))))) AND (EXISTS ( SELECT 1
   FROM "public"."event_bundles" "b"
  WHERE ("b"."id" = "event_bundle_rows"."bundle_id"))))) WITH CHECK (((EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("lower"("a"."role") = ANY (ARRAY['staff'::"text", 'admin'::"text", 'owner'::"text"]))))) AND (EXISTS ( SELECT 1
   FROM "public"."event_bundles" "b"
  WHERE ("b"."id" = "event_bundle_rows"."bundle_id")))));



CREATE POLICY "event_bundle_rows_sel_active" ON "public"."event_bundle_rows" FOR SELECT TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("lower"("a"."role") = ANY (ARRAY['staff'::"text", 'admin'::"text", 'owner'::"text"]))))) AND (EXISTS ( SELECT 1
   FROM "public"."event_bundles" "b"
  WHERE ("b"."id" = "event_bundle_rows"."bundle_id")))));
...
CREATE POLICY "event_bundles_mod_active" ON "public"."event_bundles" TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
...
CREATE POLICY "event_bundles_sel_active" ON "public"."event_bundles" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."app_accounts" "a"
  WHERE (("a"."user_id" = "auth"."uid"()) AND ("a"."is_active" = true) AND ("lower"("a"."role") = ANY (ARRAY['staff'::"text", 'admin'::"text", 'owner'::"text"]))))));
...
CREATE POLICY "frt_ins_authenticated" ON "public"."final_recipe_tags" FOR INSERT TO "authenticated" WITH CHECK (true);



CREATE POLICY "frt_sel_authenticated" ON "public"."final_recipe_tags" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "ins_authenticated" ON "public"."suppliers" FOR INSERT TO "authenticated" WITH CHECK (true);
...
CREATE POLICY "materials_sel_anon" ON "public"."materials" FOR SELECT TO "anon" USING (true);
...
CREATE POLICY "sel_authenticated" ON "public"."suppliers" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "upd_authenticated" ON "public"."suppliers" FOR UPDATE TO "authenticated" USING (true) WITH CHECK (true);
...
GRANT ALL ON FUNCTION "public"."admin_reset_all"() TO "anon";
GRANT ALL ON FUNCTION "public"."admin_reset_all"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."admin_reset_all"() TO "service_role";



REVOKE ALL ON FUNCTION "public"."admin_reset_data"("scope" "text", "caller_user_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."admin_reset_data"("scope" "text", "caller_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."admin_reset_data"("scope" "text", "caller_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."admin_reset_data"("scope" "text", "caller_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."admin_reset_settings"() TO "anon";
GRANT ALL ON FUNCTION "public"."admin_reset_settings"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."admin_reset_settings"() TO "service_role";



GRANT ALL ON FUNCTION "public"."admin_reset_suppliers"() TO "anon";
GRANT ALL ON FUNCTION "public"."admin_reset_suppliers"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."admin_reset_suppliers"() TO "service_role";



GRANT ALL ON FUNCTION "public"."app_has_role"("roles" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."app_has_role"("roles" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_has_role"("roles" "text"[]) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."app_is_admin_or_owner"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_is_admin_or_owner"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_is_admin_or_owner"() TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."app_is_contributor"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."app_is_contributor"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_is_contributor"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_is_contributor"() TO "service_role";



GRANT ALL ON FUNCTION "public"."app_is_owner"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_is_owner"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_is_owner"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."app_mark_onboarded"("p_uid" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."app_mark_onboarded"("p_uid" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_mark_onboarded"("p_uid" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."app_settings_staff_guard"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_settings_staff_guard"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_settings_staff_guard"() TO "service_role";



GRANT ALL ON FUNCTION "public"."app_settings_staff_guard_upd"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_settings_staff_guard_upd"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_settings_staff_guard_upd"() TO "service_role";



GRANT ALL ON FUNCTION "public"."app_settings_staff_only_language"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_settings_staff_only_language"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_settings_staff_only_language"() TO "service_role";



GRANT ALL ON FUNCTION "public"."app_settings_staff_only_language_ins"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_settings_staff_only_language_ins"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_settings_staff_only_language_ins"() TO "service_role";



GRANT ALL ON FUNCTION "public"."app_uid"() TO "anon";
GRANT ALL ON FUNCTION "public"."app_uid"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."app_uid"() TO "service_role";



GRANT ALL ON FUNCTION "public"."bootstrap_app_account"() TO "anon";
GRANT ALL ON FUNCTION "public"."bootstrap_app_account"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."bootstrap_app_account"() TO "service_role";



GRANT ALL ON FUNCTION "public"."bundle_types_touch"() TO "anon";
GRANT ALL ON FUNCTION "public"."bundle_types_touch"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."bundle_types_touch"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."event_bundle_rows_touch"() TO "anon";
GRANT ALL ON FUNCTION "public"."event_bundle_rows_touch"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_bundle_rows_touch"() TO "service_role";



GRANT ALL ON FUNCTION "public"."event_bundles_touch"() TO "anon";
GRANT ALL ON FUNCTION "public"."event_bundles_touch"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_bundles_touch"() TO "service_role";



GRANT ALL ON FUNCTION "public"."event_delete_full"("p_event_id" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."event_delete_full"("p_event_id" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_delete_full"("p_event_id" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."event_delete_full_many"("p_event_ids" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."event_delete_full_many"("p_event_ids" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_delete_full_many"("p_event_ids" "text"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."event_discount_rows_ensure_header"() TO "anon";
GRANT ALL ON FUNCTION "public"."event_discount_rows_ensure_header"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_discount_rows_ensure_header"() TO "service_role";



GRANT ALL ON FUNCTION "public"."event_headers_payment_defaults"() TO "anon";
GRANT ALL ON FUNCTION "public"."event_headers_payment_defaults"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_headers_payment_defaults"() TO "service_role";



GRANT ALL ON FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_price_after_discounts" bigint) TO "anon";
GRANT ALL ON FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_price_after_discounts" bigint) TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_price_after_discounts" bigint) TO "service_role";



GRANT ALL ON TABLE "public"."event_totals" TO "anon";
GRANT ALL ON TABLE "public"."event_totals" TO "authenticated";
GRANT ALL ON TABLE "public"."event_totals" TO "service_role";



GRANT ALL ON FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_totals" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_totals" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."event_totals_upsert"("p_event_id" "uuid", "p_totals" "jsonb") TO "service_role";



GRANT ALL ON FUNCTION "public"."fill_credit_customer_snapshot"() TO "anon";
GRANT ALL ON FUNCTION "public"."fill_credit_customer_snapshot"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fill_credit_customer_snapshot"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_check_month_lock"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_check_month_lock"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_check_month_lock"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_audit_log"("p_start" timestamp with time zone, "p_end" timestamp with time zone, "p_table" "text", "p_op" "text", "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_audit_log"("p_start" timestamp with time zone, "p_end" timestamp with time zone, "p_table" "text", "p_op" "text", "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_audit_log"("p_start" timestamp with time zone, "p_end" timestamp with time zone, "p_table" "text", "p_op" "text", "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."log_equipment_price_change"() TO "anon";
GRANT ALL ON FUNCTION "public"."log_equipment_price_change"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."log_equipment_price_change"() TO "service_role";



GRANT ALL ON FUNCTION "public"."log_material_price_change"() TO "anon";
GRANT ALL ON FUNCTION "public"."log_material_price_change"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."log_material_price_change"() TO "service_role";



GRANT ALL ON FUNCTION "public"."prevent_last_owner_change"() TO "anon";
GRANT ALL ON FUNCTION "public"."prevent_last_owner_change"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."prevent_last_owner_change"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."rental_equipment_apply_pricing"() TO "anon";
GRANT ALL ON FUNCTION "public"."rental_equipment_apply_pricing"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."rental_equipment_apply_pricing"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."set_updated_at_tg"() TO "anon";
GRANT ALL ON FUNCTION "public"."set_updated_at_tg"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."set_updated_at_tg"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."tg_event_company_asset_rows_set_updated_at"() TO "anon";
GRANT ALL ON FUNCTION "public"."tg_event_company_asset_rows_set_updated_at"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."tg_event_company_asset_rows_set_updated_at"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."toggle_month_lock"("p_month_key" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."toggle_month_lock"("p_month_key" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."toggle_month_lock"("p_month_key" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."transport_propagate_markup"("p_event_id" "text", "p_markup" numeric) TO "anon";
GRANT ALL ON FUNCTION "public"."transport_propagate_markup"("p_event_id" "text", "p_markup" numeric) TO "authenticated";
GRANT ALL ON FUNCTION "public"."transport_propagate_markup"("p_event_id" "text", "p_markup" numeric) TO "service_role";



GRANT ALL ON FUNCTION "public"."transport_replace_vehicle_types"("p_event_id" "text", "p_names" "text"[], "p_costs" numeric[], "p_positions" smallint[]) TO "anon";
GRANT ALL ON FUNCTION "public"."transport_replace_vehicle_types"("p_event_id" "text", "p_names" "text"[], "p_costs" numeric[], "p_positions" smallint[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."transport_replace_vehicle_types"("p_event_id" "text", "p_names" "text"[], "p_costs" numeric[], "p_positions" smallint[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."transport_rows_apply_default_markup"() TO "anon";
GRANT ALL ON FUNCTION "public"."transport_rows_apply_default_markup"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."transport_rows_apply_default_markup"() TO "service_role";



GRANT ALL ON FUNCTION "public"."transport_settings_upsert"("p_event_id" "text", "p_markup_x" numeric) TO "anon";
GRANT ALL ON FUNCTION "public"."transport_settings_upsert"("p_event_id" "text", "p_markup_x" numeric) TO "authenticated";
GRANT ALL ON FUNCTION "public"."transport_settings_upsert"("p_event_id" "text", "p_markup_x" numeric) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_hr_staff_updated_at"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_hr_staff_updated_at"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_hr_staff_updated_at"() TO "service_role";
...
GRANT ALL ON TABLE "public"."_app_settings_backup" TO "service_role";
...
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE "public"."app_accounts" TO "supabase_auth_admin";
...
GRANT ALL ON TABLE "public"."app_settings_staff_whitelist" TO "service_role";
...
GRANT SELECT,USAGE ON SEQUENCE "public"."audit_log_id_seq" TO "supabase_auth_admin";



GRANT ALL ON TABLE "public"."bundle_types" TO "anon";
GRANT ALL ON TABLE "public"."bundle_types" TO "authenticated";
GRANT ALL ON TABLE "public"."bundle_types" TO "service_role";



GRANT ALL ON TABLE "public"."cash_ledger_deposits" TO "anon";
GRANT ALL ON TABLE "public"."cash_ledger_deposits" TO "authenticated";
GRANT ALL ON TABLE "public"."cash_ledger_deposits" TO "service_role";
...
GRANT ALL ON TABLE "public"."cashout" TO "anon";
GRANT ALL ON TABLE "public"."cashout" TO "authenticated";
GRANT ALL ON TABLE "public"."cashout" TO "service_role";
...
GRANT SELECT,USAGE ON SEQUENCE "public"."categories_id_seq" TO "supabase_auth_admin";



GRANT ALL ON TABLE "public"."event_headers" TO "anon";
GRANT ALL ON TABLE "public"."event_headers" TO "authenticated";
GRANT ALL ON TABLE "public"."event_headers" TO "service_role";



GRANT ALL ON TABLE "public"."catering_event_list_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."catering_event_list_vw" TO "service_role";



GRANT ALL ON TABLE "public"."event_payment_state" TO "anon";
GRANT ALL ON TABLE "public"."event_payment_state" TO "authenticated";
GRANT ALL ON TABLE "public"."event_payment_state" TO "service_role";



GRANT ALL ON TABLE "public"."catering_event_pay_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."catering_event_pay_vw" TO "service_role";



GRANT ALL ON TABLE "public"."contract_template_settings" TO "anon";
GRANT ALL ON TABLE "public"."contract_template_settings" TO "authenticated";
GRANT ALL ON TABLE "public"."contract_template_settings" TO "service_role";
...
GRANT ALL ON TABLE "public"."credit_totals_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."credit_totals_vw" TO "service_role";



GRANT ALL ON TABLE "public"."credits_with_totals_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."credits_with_totals_vw" TO "service_role";



GRANT ALL ON TABLE "public"."crm_agreements" TO "anon";
GRANT ALL ON TABLE "public"."crm_agreements" TO "authenticated";
GRANT ALL ON TABLE "public"."crm_agreements" TO "service_role";
...
GRANT ALL ON TABLE "public"."crm_partners" TO "anon";
GRANT ALL ON TABLE "public"."crm_partners" TO "authenticated";
GRANT ALL ON TABLE "public"."crm_partners" TO "service_role";



GRANT ALL ON TABLE "public"."crm_payouts" TO "anon";
GRANT ALL ON TABLE "public"."crm_payouts" TO "authenticated";
GRANT ALL ON TABLE "public"."crm_payouts" TO "service_role";
...
GRANT ALL ON TABLE "public"."daily_report_bank_transfers" TO "anon";
GRANT ALL ON TABLE "public"."daily_report_bank_transfers" TO "authenticated";
GRANT ALL ON TABLE "public"."daily_report_bank_transfers" TO "service_role";
...
GRANT ALL ON TABLE "public"."deposit_payments" TO "anon";
GRANT ALL ON TABLE "public"."deposit_payments" TO "authenticated";
...
GRANT ALL ON TABLE "public"."dish_categories" TO "service_role";
...
GRANT ALL ON SEQUENCE "public"."dish_categories_id_seq" TO "service_role";
GRANT SELECT,USAGE ON SEQUENCE "public"."dish_categories_id_seq" TO "supabase_auth_admin";
...
GRANT SELECT,USAGE ON SEQUENCE "public"."equipment_categories_id_seq" TO "supabase_auth_admin";



GRANT ALL ON TABLE "public"."equipment_price_history" TO "service_role";
GRANT SELECT ON TABLE "public"."equipment_price_history" TO "authenticated";



GRANT ALL ON TABLE "public"."event_bundle_rows" TO "authenticated";
GRANT ALL ON TABLE "public"."event_bundle_rows" TO "service_role";



GRANT ALL ON TABLE "public"."event_bundles" TO "authenticated";
GRANT ALL ON TABLE "public"."event_bundles" TO "service_role";



GRANT ALL ON TABLE "public"."event_company_asset_rows" TO "anon";
GRANT ALL ON TABLE "public"."event_company_asset_rows" TO "authenticated";
GRANT ALL ON TABLE "public"."event_company_asset_rows" TO "service_role";



GRANT ALL ON TABLE "public"."event_contracts" TO "anon";
GRANT ALL ON TABLE "public"."event_contracts" TO "authenticated";
GRANT ALL ON TABLE "public"."event_contracts" TO "service_role";



GRANT ALL ON TABLE "public"."event_discount_rows" TO "anon";
GRANT ALL ON TABLE "public"."event_discount_rows" TO "authenticated";
GRANT ALL ON TABLE "public"."event_discount_rows" TO "service_role";



GRANT ALL ON TABLE "public"."event_equipment_rows" TO "anon";
GRANT ALL ON TABLE "public"."event_equipment_rows" TO "authenticated";
GRANT ALL ON TABLE "public"."event_equipment_rows" TO "service_role";



GRANT ALL ON TABLE "public"."event_extra_fee_rows" TO "anon";
GRANT ALL ON TABLE "public"."event_extra_fee_rows" TO "authenticated";
GRANT ALL ON TABLE "public"."event_extra_fee_rows" TO "service_role";



GRANT ALL ON TABLE "public"."event_staff_rows" TO "anon";
GRANT ALL ON TABLE "public"."event_staff_rows" TO "authenticated";
GRANT ALL ON TABLE "public"."event_staff_rows" TO "service_role";



GRANT ALL ON TABLE "public"."event_staff_settings" TO "anon";
GRANT ALL ON TABLE "public"."event_staff_settings" TO "authenticated";
GRANT ALL ON TABLE "public"."event_staff_settings" TO "service_role";



GRANT ALL ON TABLE "public"."event_transport_rows" TO "anon";
GRANT ALL ON TABLE "public"."event_transport_rows" TO "authenticated";
GRANT ALL ON TABLE "public"."event_transport_rows" TO "service_role";



GRANT ALL ON TABLE "public"."event_transport_settings" TO "anon";
GRANT ALL ON TABLE "public"."event_transport_settings" TO "authenticated";
GRANT ALL ON TABLE "public"."event_transport_settings" TO "service_role";



GRANT ALL ON TABLE "public"."event_transport_vehicle_types" TO "anon";
GRANT ALL ON TABLE "public"."event_transport_vehicle_types" TO "authenticated";
GRANT ALL ON TABLE "public"."event_transport_vehicle_types" TO "service_role";



GRANT ALL ON TABLE "public"."final_recipe_items" TO "service_role";
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE "public"."final_recipe_items" TO "authenticated";



GRANT ALL ON TABLE "public"."final_recipes" TO "service_role";
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE "public"."final_recipes" TO "authenticated";



GRANT ALL ON TABLE "public"."final_list_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."final_list_vw" TO "service_role";



GRANT ALL ON TABLE "public"."final_recipe_tags" TO "service_role";
...
GRANT ALL ON TABLE "public"."final_recipe_tags_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."final_recipe_tags_vw" TO "service_role";



GRANT ALL ON TABLE "public"."gift_vouchers" TO "anon";
GRANT ALL ON TABLE "public"."gift_vouchers" TO "authenticated";
GRANT ALL ON TABLE "public"."gift_vouchers" TO "service_role";



GRANT ALL ON TABLE "public"."hr_staff" TO "anon";
GRANT ALL ON TABLE "public"."hr_staff" TO "authenticated";
GRANT ALL ON TABLE "public"."hr_staff" TO "service_role";



GRANT ALL ON TABLE "public"."hr_staff_branches" TO "anon";
GRANT ALL ON TABLE "public"."hr_staff_branches" TO "authenticated";
GRANT ALL ON TABLE "public"."hr_staff_branches" TO "service_role";



GRANT ALL ON TABLE "public"."loyalty_card_transactions" TO "anon";
GRANT ALL ON TABLE "public"."loyalty_card_transactions" TO "authenticated";
GRANT ALL ON TABLE "public"."loyalty_card_transactions" TO "service_role";
...
GRANT SELECT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."material_price_history" TO "service_role";
...
GRANT ALL ON TABLE "public"."materials_vat_backup" TO "anon";
GRANT ALL ON TABLE "public"."materials_vat_backup" TO "authenticated";
GRANT ALL ON TABLE "public"."materials_vat_backup" TO "service_role";
...
GRANT ALL ON TABLE "public"."month_locks" TO "anon";
GRANT ALL ON TABLE "public"."month_locks" TO "authenticated";
GRANT ALL ON TABLE "public"."month_locks" TO "service_role";



GRANT ALL ON TABLE "public"."prep_recipes" TO "service_role";
...
GRANT ALL ON TABLE "public"."prep_list_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."prep_list_vw" TO "service_role";
...
GRANT ALL ON TABLE "public"."prep_list_with_uom_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."prep_list_with_uom_vw" TO "service_role";



GRANT ALL ON TABLE "public"."prep_recipe_items" TO "service_role";
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE "public"."prep_recipe_items" TO "authenticated";



GRANT ALL ON TABLE "public"."prep_recipe_tags" TO "service_role";
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE "public"."prep_recipe_tags" TO "authenticated";



GRANT ALL ON TABLE "public"."prepaid_cards" TO "anon";
GRANT ALL ON TABLE "public"."prepaid_cards" TO "authenticated";
GRANT ALL ON TABLE "public"."prepaid_cards" TO "service_role";



GRANT ALL ON TABLE "public"."provider_branches" TO "anon";
GRANT ALL ON TABLE "public"."provider_branches" TO "authenticated";
GRANT ALL ON TABLE "public"."provider_branches" TO "service_role";



GRANT ALL ON SEQUENCE "public"."recipe_categories_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."recipe_categories_id_seq" TO "service_role";
GRANT SELECT,USAGE ON SEQUENCE "public"."recipe_categories_id_seq" TO "supabase_auth_admin";



GRANT ALL ON TABLE "public"."rental_equipment" TO "service_role";
...
GRANT SELECT,USAGE ON SEQUENCE "public"."tags_id_seq" TO "supabase_auth_admin";



GRANT ALL ON TABLE "public"."transport_defaults" TO "anon";
GRANT ALL ON TABLE "public"."transport_defaults" TO "authenticated";
GRANT ALL ON TABLE "public"."transport_defaults" TO "service_role";



GRANT ALL ON TABLE "public"."view_deposit_remaining" TO "authenticated";
GRANT ALL ON TABLE "public"."view_deposit_remaining" TO "service_role";
...
GRANT ALL ON TABLE "public"."wastage_entries" TO "anon";
GRANT ALL ON TABLE "public"."wastage_entries" TO "authenticated";
GRANT ALL ON TABLE "public"."wastage_entries" TO "service_role";



GRANT ALL ON TABLE "public"."wastage_monthly_vw" TO "authenticated";
GRANT ALL ON TABLE "public"."wastage_monthly_vw" TO "service_role";
